...
func (v *Widget) SetFromPixbuf(pixbuf *gdkpixbuf.Pixbuf) {
	temp := Image(struct{ Misc }{Misc(struct{ Widget }{*v})})

	C.gtk_image_set_from_pixbuf(IMAGE(&temp), (*C.GdkPixbuf)(unsafe.Pointer(pixbuf.GPixbuf)))
}
...
func (v *Widget) SetFraction(fraction float64) {
	temp := ProgressBar(struct{ Widget }{*v})
	C.gtk_progress_bar_set_fraction(PROGRESS_BAR(&temp), gdouble(fraction))
}
...
func (v *Widget) GetFraction() float64 {
	temp := struct {
		Widget
	}{
		*v,
	}
	flag := ProgressBar(temp)

	r := C.gtk_progress_bar_get_fraction(PROGRESS_BAR(&flag))
	return float64(r)
}
